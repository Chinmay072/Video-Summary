...
def get_search_terms(text):
    ...
    Format: Return only the terms, one per line, no numbers or extra text.
    Example format:
    neural networks
    data visualization
    cloud computing
...
def download_theme_images(query, output_dir, num_images=5):
    ...
    if not downloaded_images:
        print("Falling back to default technology images...")
        # Create a solid color image as fallback
        fallback_img = Image.new('RGB', (800, 600), (30, 30, 30))
        fallback_path = os.path.join(output_dir, "fallback.jpg")
        fallback_img.save(fallback_path)
        downloaded_images = [fallback_path] * num_images
